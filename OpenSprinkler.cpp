...
WiFiConfig OpenSprinkler::wifi_config =
    {WIFI_MODE_AP, "", ""};
...
time_t OpenSprinkler::now_tz()
    {
    return now()
	    + (int32_t) 3600 / 4 * (int32_t) (options[OPTION_TIMEZONE] - 48);
    }
...
byte OpenSprinkler::start_network()
    ...
    ether.hisport = (unsigned int) (options[OPTION_HTTPPORT_1] << 8)
	    + (unsigned int) options[OPTION_HTTPPORT_0];
#ifndef OSMICRO
    if (options[OPTION_USE_DHCP])
	{
	// set up DHCP
	...
	if (!ether.staticSetup(staticip, gateway, dns))
	    return 0;
	}
#else
    // set up DHCP
...
void OpenSprinkler::lcd_start()
    ...
    // turn on lcd
    lcd.init(1, PIN_LCD_RS, 255, PIN_LCD_EN, PIN_LCD_D4, PIN_LCD_D5, PIN_LCD_D6,
	    PIN_LCD_D7, 0, 0, 0, 0);
    lcd.begin();
...
void OpenSprinkler::rainsensor_status()
    ...
    if (options[OPTION_SENSOR_TYPE] != SENSOR_TYPE_RAIN)
	return;
    status.rain_sensed = (
	    digitalRead(PIN_RAINSENSOR) == options[OPTION_RAINSENSOR_TYPE] ?
		    0 : 1);
    }
...
void OpenSprinkler::get_station_name(byte sid, char tmp[])
    ...
    nvm_read_block(tmp,
	    (void*) (ADDR_NVM_STN_NAMES + (int) sid * STATION_NAME_SIZE),
	    STATION_NAME_SIZE);
    }
...
void OpenSprinkler::switch_special_station(byte sid, byte value)
    ...
    if (station_attrib_bits_read(ADDR_NVM_STNSPE + (sid >> 3))
	    & (1 << (sid & 0x07)))
	{
...
void OpenSprinkler::switch_remotestation(RemoteStationData *data, bool turnon)
    ...
    // MAX_NUM_STATIONS is the refresh cycle
    uint16_t timer =
	    options[OPTION_SPE_AUTO_REFRESH] ? 2 * MAX_NUM_STATIONS : 64800;
    bf.emit_p(PSTR("?pw=$E&sid=$D&en=$D&t=$D"),
    ADDR_NVM_PASSWORD, (int) hex2ulong(data->sid, sizeof(data->sid)), turnon,
	    timer);
...
void OpenSprinkler::options_setup()
    ...
    byte curr_ver = nvm_read_byte(
	    (byte*) (ADDR_NVM_OPTIONS + OPTION_FW_VERSION));
...
#endif
	// 0. wipe out nvm
	for (i = 0; i < TMP_BUFFER_SIZE; i++)
	    tmp_buffer[i] = 0;
	for (i = 0; i < NVM_SIZE; i += TMP_BUFFER_SIZE)
	    {
	    int nbytes = ((NVM_SIZE - i) > TMP_BUFFER_SIZE) ?
	    TMP_BUFFER_SIZE :
							      (NVM_SIZE - i);
	...
	nvm_write_block(DEFAULT_PASSWORD, (void*) ADDR_NVM_PASSWORD,
		strlen(DEFAULT_PASSWORD) + 1);
	nvm_write_block(DEFAULT_LOCATION, (void*) ADDR_NVM_LOCATION,
		strlen(DEFAULT_LOCATION) + 1);
	...
	nvm_write_block(DEFAULT_WEATHER_KEY, (void*) ADDR_NVM_WEATHER_KEY,
		strlen(DEFAULT_WEATHER_KEY) + 1);

	// 3. reset station names and special attributes, default Sxx
	tmp_buffer[0] = 'S';
	tmp_buffer[3] = 0;
	for (i = ADDR_NVM_STN_NAMES, sn = 1; i < ADDR_NVM_MAS_OP; i +=
	STATION_NAME_SIZE, sn++)
	    {
	...
	for (i = 0; i < MAX_NUM_STATIONS; i++)
	    {
	    write_to_file(stns_filename, tmp_buffer, stepsize, i * stepsize,
	    false);
	    }
#endif
	...
	nvm_write_block(tmp_buffer, (void*) ADDR_NVM_MAS_OP,
	MAX_EXT_BOARDS + 1);
	nvm_write_block(tmp_buffer, (void*) ADDR_NVM_STNSEQ,
	MAX_EXT_BOARDS + 1);
...
void OpenSprinkler::lcd_print_pgm(PGM_P str)
	...
	void OpenSprinkler::lcd_print_station(byte line, char c)
	    ...
	    if (status.rain_delayed
		    || (status.rain_sensed
			    && options[OPTION_SENSOR_TYPE] == SENSOR_TYPE_RAIN))
		{
		lcd.write(3);
		}
	...
	void OpenSprinkler::lcd_print_option(int i)
	    ...
	    if (i == OPTION_WATER_PERCENTAGE)
	    lcd_print_pgm(PSTR("%"));
	    else if (i == OPTION_MASTER_ON_ADJ || i == OPTION_MASTER_OFF_ADJ
		    || i == OPTION_MASTER_ON_ADJ_2 || i == OPTION_MASTER_OFF_ADJ_2)
	...
	byte OpenSprinkler::button_read_busy(byte pin_butt, byte waitmode, byte butt,
		byte is_holding)
	    {

	    int hold_time = 0;

	    if (waitmode == BUTTON_WAIT_NONE
		    || (waitmode == BUTTON_WAIT_HOLD && is_holding))
		{
	    ...
	    while (digitalReadExt(pin_butt) == 0
		    && (waitmode == BUTTON_WAIT_RELEASE
			    || (waitmode == BUTTON_WAIT_HOLD
				    && hold_time < BUTTON_HOLD_MS)))
		{
	...
	void OpenSprinkler::ui_set_options(int oid)
		...
		switch (button & BUTTON_MASK)
		    ...
		    if (i == OPTION_FW_VERSION || i == OPTION_HW_VERSION
			    || i == OPTION_FW_MINOR || i == OPTION_HTTPPORT_0
			    || i == OPTION_HTTPPORT_1 || i == OPTION_PULSE_RATE_0
			    || i == OPTION_PULSE_RATE_1)
		    break; // ignore non-editable options
		    if (pgm_read_byte(op_max+i) != options[i])
		    options[i]++;
		    break;
			...
			if (i == OPTION_USE_DHCP && options[i])
			i += 9;// if use DHCP, skip static ip set
			else if (i == OPTION_HTTPPORT_0)
			i += 2;// skip OPTION_HTTPPORT_1
			else if (i == OPTION_PULSE_RATE_0)
			i += 2;// skip OPTION_PULSE_RATE_1
			else if (i == OPTION_SENSOR_TYPE&& options[i]!=SENSOR_TYPE_RAIN)
			i += 2;// if not using rain sensor, skip rain sensor type
			else if (i == OPTION_MASTER_STATION && options[i] == 0)
			i += 3;// if not using master station, skip master on/off adjust
			else if (i == OPTION_MASTER_STATION_2 && options[i] == 0)
			i += 3;// if not using master2, skip master2 on/off adjust
			else
			    {
			    i = (i + 1) % NUM_OPTIONS;
			    }
			if (i == OPTION_SEQUENTIAL_RETIRED)
			i++;
#if defined(__AVR_ATmega1284P__) || defined(__AVR_ATmega1284__) || defined(ESP8266)
