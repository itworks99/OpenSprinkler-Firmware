...
void send_packet(bool final = false)
    ...
    if (final)
	{
	ether.httpServerReply_with_flags(bfill.position(),
...
#endif
    {
    if (os.options[OPTION_IGNORE_PASSWORD])
	return true;
...
void server_json_station_special()
    ...
    handle_return(HTML_OK);
#else
    handle_return(HTML_PAGE_NOT_FOUND);
#endif
    }
...
void server_json_options_main()
	...
	if (oid == OPTION_SEQUENTIAL_RETIRED)
	    continue;
	if (oid == OPTION_DEVICE_ID && os.status.has_hwmac)
	    continue; // do not send DEVICE ID if hardware MAC exists
...
#else
    bfill.emit_p(PSTR(",\"dexp\":$D,\"mexp\":$D,\"hwt\":$D}"), 0,
...
void server_json_programs_main()
    ...
    bfill.emit_p(
	    PSTR(
		    "\"nprogs\":$D,\"nboards\":$D,\"mnp\":$D,\"mnst\":$D,\"pnsize\":$D,\"pd\":["),
	    pd.nprograms, os.nboards, MAX_NUMBER_PROGRAMS, MAX_NUM_STARTTIMES,
	    PROGRAM_NAME_SIZE);
    byte pid, i;
    ProgramStruct prog;
...
void server_view_scripturl()
    ...
    bfill.emit_p(
	    PSTR(
		    "<form name=of action=cu method=get><table cellspacing=\"12\"><tr><td><b>JavaScript</b>:</td><td><input type=text size=40 maxlength=40 value=\"$E\" name=jsp></td></tr><tr><td>Default:</td><td>$S</td></tr><tr><td><b>Weather</b>:</td><td><input type=text size=40 maxlength=40 value=\"$E\" name=wsp></td></tr><tr><td>Default:</td><td>$S</td></tr><tr><td><b>Password</b>:</td><td><input type=password size=32 name=pw> <input type=submit></td></tr></table></form><script src=https://ui.opensprinkler.com/js/hasher.js></script>"),
	    ADDR_NVM_JAVASCRIPTURL, DEFAULT_JAVASCRIPT_URL, ADDR_NVM_WEATHERURL,
	    DEFAULT_WEATHER_URL);
    handle_return(HTML_OK);
    }
...
void server_json_controller_main()
    ...
    bfill.emit_p(
	    PSTR(
		    "\"devt\":$L,\"nbrd\":$D,\"en\":$D,\"rd\":$D,\"rs\":$D,\"rdst\":$L,"
			    "\"loc\":\"$E\",\"wtkey\":\"$E\",\"sunrise\":$D,\"sunset\":$D,\"eip\":$L,\"lwc\":$L,\"lswc\":$L,"
			    "\"lrun\":[$D,$D,$D,$L],"), curr_time, os.nboards,
	    os.status.enabled, os.status.rain_delayed, os.status.rain_sensed,
	    os.nvdata.rd_stop_time,
	    ADDR_NVM_LOCATION,
	    ADDR_NVM_WEATHER_KEY, os.nvdata.sunrise_time, os.nvdata.sunset_time,
	    os.nvdata.external_ip, os.checkwt_lasttime,
	    os.checkwt_success_lasttime, pd.lastrun.station, pd.lastrun.program,
...
void server_change_scripturl()
	...
	nvm_write_block(tmp_buffer, (void *) ADDR_NVM_JAVASCRIPTURL,
		strlen(tmp_buffer) + 1);
	}
	...
	string_remove_space(tmp_buffer);
	nvm_write_block(tmp_buffer, (void *) ADDR_NVM_WEATHERURL,
		strlen(tmp_buffer) + 1);
	}
    handle_return(HTML_REDIRECT_HOME);
    }
...
void server_change_options()
	...
	if (max_value == 1)
	    os.options[oid] = 0;  // set a bool variable to 0 first
	char tbuf2[5] =
	    {
	    'o', 0, 0, 0, 0
	    };
	...
	if (findKeyVal(p, tmp_buffer, TMP_BUFFER_SIZE, tbuf2))
		...
		if (oid == OPTION_MASTER_OFF_ADJ
			|| oid == OPTION_MASTER_OFF_ADJ_2
			|| oid == OPTION_MASTER_ON_ADJ
			|| oid == OPTION_MASTER_ON_ADJ_2
			|| oid == OPTION_STATION_DELAY_TIME)
		    {
	...
	if (os.options[oid] != prev_value)
	    {	// if value has changed
	    if (oid == OPTION_TIMEZONE || oid == OPTION_USE_NTP)
		time_change = true;
	    if (oid >= OPTION_NTP_IP1 && oid <= OPTION_NTP_IP4)
		time_change = true;
	    if (oid >= OPTION_USE_DHCP && oid <= OPTION_HTTPPORT_1)
		network_change = true;
	    if (oid == OPTION_DEVICE_ID)
		network_change = true;
	    if (oid == OPTION_USE_WEATHER)
		weather_change = true;
	    }
	}
	...
	if (strcmp_to_nvm(tmp_buffer, ADDR_NVM_LOCATION))
	    { // if location has changed
	    nvm_write_block(tmp_buffer, (void*) ADDR_NVM_LOCATION,
		    strlen(tmp_buffer) + 1);
	    weather_change = true;
	    }
	}
	...
	if (strcmp_to_nvm(tmp_buffer, ADDR_NVM_WEATHER_KEY))
	    {  // if weather key has changed
	    nvm_write_block(tmp_buffer, (void*) ADDR_NVM_WEATHER_KEY,
		    strlen(tmp_buffer) + 1);
	    weather_change = true;
	    }
	}
...
void server_change_password()
	...
	if (findKeyVal(p, tbuf2, TMP_BUFFER_SIZE, PSTR("cpw"), true)
		&& strncmp(tmp_buffer, tbuf2, MAX_USER_PASSWORD) == 0)
	    {
#if defined(DEMO)
	    handle_return(HTML_SUCCESS);
#endif
	    ...
	    nvm_write_block(tmp_buffer, (void*) ADDR_NVM_PASSWORD,
		    strlen(tmp_buffer) + 1);
	    handle_return(HTML_SUCCESS);
	    }
	else
...
void server_json_status_main()
	...
	{
	bfill.emit_p(PSTR("$D"),
		(os.station_bits[(sid >> 3)] >> (sid & 0x07)) & 1);
	if (sid != os.nstations - 1)
	    bfill.emit_p(PSTR(","));
	}
...
void server_change_manual()
	...
	if (findKeyVal(p, tmp_buffer, TMP_BUFFER_SIZE, PSTR("t"), true))
	    ...
	    if (sqi != 0xFF)
		{  // if we, we will overwrite the schedule
		q = pd.queue + sqi;
		}
	    else
...
void handle_web_request(char *p)
	...
	for (i = 0; i < sizeof(urls) / sizeof(URLHandler); i++)
		...
		if (com[0] == 's' && com[1] == 'u')
			...
			{
			print_json_header();
			bfill.emit_p(PSTR("\"$F\":$D}"), op_json_names + 0,
				os.options[0]);
			ret = HTML_OK;
			}
		    else
